...
class Environnement(Model):
    ...
    def actualize_week(self):
        self.day=0
        if(self.season<self.max_season-1 and self.seasonDirection):
            self.season+=1
        else:
            self.seasonDirection = False
            self.season-=1
            if self.season<=0:
                self.seasonDirection = True
        for i in self.schedule.agents:
    ...
    def embody_changes(self):
        """
        When using the interface, this will add/remove agents
            must not violate the fact that there is always at least a
            producer for each hour
        Returns
        -------
        None.
    ...
    def actualize_day(self):
        ...
        # verifciation if there exists a producer for each hour
    ...
    def rday(self):
        ...
        self.vday+=1
        """
        for i in self.marketPlace.market:
            print(i)
        for i in self.marketPlace.prices_request:
            print(i)
        for i in self.marketPlace.price_sent:
            print(i)
        """
        self.actualize_day()
