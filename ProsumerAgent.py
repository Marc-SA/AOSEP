...
class ProsumerAgent(Agent):
    ...
    #to ensure that there's always a energy producer
    GreedyCapitalist = [1]
    ...
    def get_dict(self):
        dic={}
        dic["unique_id"]=str(self.unique_id)
        dic["weights"]=str(self.weights)
        dic["treshold"]=str(self.treshold)
        dic["jobs"]=str(self.jobs)
        dic["sjobs"]=str(self.Sjobs)
        dic["fossileprice"]=str(self.fossile_price)
        dic["nuclear_price"]=str(self.nuclear_price)
        dic["solar_price"]=str(self.solar_price)
        dic["table_wind"]=str(self.table_wind)
        dic["is_cons"]=str(self.is_consumerr)
        dic["producer_table"]=str(self.producer_table)
        return dic

    def change_prices(self):
        """
        Changes the prices for each type of energy : fossil, wind, nuclear, solar
            this is done almost randomly given what we have put as class variables above
        Returns
        -------
        None.
    ...
    def __init__(self, unique_id,producer_table,is_cons ,model,x,y, weights= [0.8,0.1,0.1], treshold=0.7,
        ...
        """ Automate to modelise the system - see Prosumer FSM -per day
        """
        #Overall machine
    ...
    def purchase(self):
        ...
        for k in range(len(self.jobs)):
            #print("JOBS",self.model.day,self.jobs[k],)
            purchase.append(self.intelligent_purchase(optimal_prices,self.jobs[k][self.model.day]))
